/*
 * ============================================================================
 * GRANT-FREE SCHEDULING - PHASE 3: gNB Scheduler Functions
 * File: openair2/LAYER2/NR_MAC_gNB/gNB_scheduler_gf.c (NEW FILE)
 * 
                               ...
                               nr_gf_gnb_slot_info_t *info)
{
  ...
  // Quick check using bitmap (optional optimization)
  // if (!IS_GF_SLOT(gf_mgr, slot)) {
  //   return 0;
  // }
  
  // Get frame configuration
  NR_ServingCellConfigCommon_t *scc = gNB->common_channels[0].ServingCellConfigCommon;
  int slots_per_frame = nr_slots_per_frame[*scc->ssbSubcarrierSpacing];
  uint64_t abs_slot = GET_ABSOLUTE_SLOT(frame, slot, slots_per_frame);
  
  // Check if this slot is UL (for TDD)
  // In OAI, check slot type from frame configuration
  // For simplicity, assume caller has verified this is a UL slot
                                ...
                                int slot)
{
  ...
  uint16_t l_prime = get_l_prime(gf_config->nr_of_symbols,
                                  ...
                                  pusch_dmrs_AdditionalPosition_pos0,  // Single DMRS
                                  pusch_dmrs_len1,
                                  gf_config->start_symbol,
                                   ...
                                   nr_gf_gnb_slot_info_t *info)
{
  ...
  for (int i = 0; i < info->num_expected; i++) {
    ...
    // Get PUSCH list for this slot
    // Note: OAI structure varies by version - adapt as needed
    NR_gNB_PUSCH_t *pusch_info = &gNB->ulsch[gf->ue_id][0];
    
    // Mark as expecting reception
    pusch_info->active = 1;
    pusch_info->frame = frame;
    pusch_info->slot = slot;
    
    // Configure PUSCH PDU
    // Note: Actual integration depends on OAI FAPI interface
    nfapi_nr_pusch_pdu_t pusch_pdu;
    nr_gf_gnb_config_pusch_pdu(gNB, gf, &pusch_pdu, frame, slot);
    
    // Add to UL_tti_req (FAPI structure for PHY)
    // This is simplified - actual OAI code uses more complex structures
    // Add PUSCH to gNB->UL_tti_req for PHY processing
                           ...
                           int8_t sinr_db)
{
  ...
  if (crc_valid) {
    ...
    // Process MAC PDU (extract BSR, SDUs, forward to upper layers)
    // Use existing OAI function:
                                       ...
                                       int rx_slot)
{
  ...
  // For simplicity, use fixed delay of 4 slots
  int k1 = 4;
  ...
  // Store feedback info
  // Note: In real implementation, use proper data structure
  // Here we store in UE's scheduling control
  NR_UE_info_t *UE_info = &gNB->UE_info;
  NR_UE_sched_ctrl_t *sched_ctrl = &UE_info->UE_sched_ctrl[ue_id];
  
  // Store pending feedback (simplified - extend sched_ctrl as needed)
  // sched_ctrl->gf_harq_feedback.rnti = ...
  // sched_ctrl->gf_harq_feedback.harq_process_id = harq_pid;
  // sched_ctrl->gf_harq_feedback.ack = ack;
  // sched_ctrl->gf_harq_feedback.feedback_frame = fb_frame;
  // sched_ctrl->gf_harq_feedback.feedback_slot = fb_slot;
  // sched_ctrl->gf_harq_feedback.feedback_sent = false;
...
{
  // Check all UEs for pending GF HARQ feedback
  NR_UE_info_t *UE_info = &gNB->UE_info;
  ...
  for (int ue_id = 0; ue_id < MAX_MOBILES_PER_GNB; ue_id++) {
    ...
    // Check if feedback is due for this slot
    // Simplified - actual implementation needs proper structure
    // if (sched_ctrl->gf_harq_feedback.feedback_frame == frame &&
    //     sched_ctrl->gf_harq_feedback.feedback_slot == slot &&
    //     !sched_ctrl->gf_harq_feedback.feedback_sent) {
    //   
    //   // Send feedback via DCI format 0_0
    //   // Or use other mechanism as configured
    //   
    //   sched_ctrl->gf_harq_feedback.feedback_sent = true;
    // }
  }
}
